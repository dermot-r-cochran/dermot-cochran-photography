...
- a `keywords:` that is not a list, names a word outside the vocabulary at
  the top of the script, or repeats one — a keyword is an overlapping tag, so
  an unknown word wouldn't misfile a photo the way a bad category does, but
  it would build a `/keywords/` page holding one photo under a name nobody
  else will spell the same way. A keyword carried by fewer than four photos
  only **warns**: the floor is a judgement (retire the word, or tag the photos
  that should carry it), and CLAUDE.md's "Keywords" says why it is not a gate
...
stripping), `groupPhotosBy` (including the list-valued case `keywords:`
