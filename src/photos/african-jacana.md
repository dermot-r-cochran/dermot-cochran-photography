---
layout: photo.njk
title: "African Jacana"
category: "Wildlife"
location: "Amboseli, Kenya"
year: 2025
album: "Amboseli, October 2025"
image: "african-jacana.jpg"
alt: "An African jacana standing among short wet marsh grass, its deep chestnut body turned side-on, white face and throat below a black crown, pale blue bill, and its long thin legs disappearing into the weed below"
order: 127
competitions: [DCC]
subjects: [Birds, Waterbirds]
---

The background here has been softened in processing. Straight out of the camera
the marsh grass was as sharp as the bird and the same colour, so the jacana
simply disappeared into it. Cropping and blurring is the reason this one carries
only the DCC tag — the nature sections do not allow blurring to hide elements of
a scene, and rightly so.
