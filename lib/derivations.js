...
// both and never an in-between.
//
// The scope is plants and natural subjects: Nature and Macro only (Dermot's
// ruling, 24 September 2026). Animals were in scope until then, filed Wild by
// category, and that put the row on every Wildlife page to repeat what the
// category already said - 89 pages, none of them sorted by it. "Cultivated"
// is a word for plants: the animal version of the question is wild, captive
// or domestic, and the category rules already answer it (pure wildlife ->
// Wildlife, free-living in a built setting -> Urban Wildlife, captive or any
// hand of man -> Documentary). A zoo animal labelled Cultivated would read
// oddly and one labelled Wild would be false, so animals have no bucket here.
//
// Within that scope the tag is exact, because IPF-Nature is a COMPOUND test -
// wild AND no human element AND not cultivated AND not feral - and for a plant
// the only realistic way to fail it is cultivation. (For an animal it usually
// fails on a vehicle track or a building in frame, which is one more reason the
// tag was never the animal signal.) A Nature frame with an animal in it, such
// as a hoverfly on a thistle, is labelled by the plant, which is the reading
// the tag already gives.
//
// Every other category is out of scope and appears in neither bucket -
...
// Subject nesting, declared once (Dermot, 21 September 2026). A kind sits
// inside a wider kind, and the wider page rolls the narrower one up: a
// photograph tagged Lions appears on the Wild Cats page without being tagged
// Wild Cats. Before this the hierarchy was encoded by tagging both, which is
// what grew `Birds` to 51 photographs - a quarter of the site - before it was
// split. Tag the narrowest kind that fits; the parent is implied, and tagging
// both is redundant and rejected by scripts/validate-photos.js.
...
// Related pages that are not nested (22 September 2026). `Birds` is the
// residual term since the split of 21 September - the birds no kind covers -
// and deliberately not a parent, because rolling the kinds back up would put
// it at about 51 photographs again. That left a visitor who arrived at Birds
// seeing nine photos with no sign that forty more were a click away. A
// see-also line links the pages without re-inflating the one it sits on.
// Listed in the order they render; scripts/validate-photos.js fails any name
// outside the subject vocabulary, since a stale one would be a dead link.
// Flowers got the same line the same day: Blossom sits inside Trees, not
// Flowers, so a flowering tree is a click away rather than on the page, and
// Garden Flowers - nested inside Flowers, so already on it - is linked as the
// narrower view.
const SUBJECT_SEE_ALSO = {
  Birds: ["Birds of Prey", "Seabirds", "Waterbirds", "Birds in Flight"],
  Flowers: ["Garden Flowers", "Blossom"]
};

// Every subject a photograph belongs to: the ones it carries plus the wider
// kinds those sit inside, in order and without duplicates. Loop-guarded, so a
// mistake in SUBJECT_PARENTS cannot hang the build.
function subjectsWithParents(subjects) {
  const out = [];
  for (const subject of subjects || []) {
    ...
    while (current && !seen.has(current)) {
      ...
      current = SUBJECT_PARENTS[current];
    }
  }
  return out;
}
...
// src/year, src/albums, src/country, src/subjects) via
...
function groupPhotosBy(photos, keyOf) {
  ...
  for (const photo of photos) {
    // A list value (the `subjects:` field) files the photo under every
    // entry. Every other axis is single-valued: album and location are
    // exclusive by meaning, subjects overlap by meaning.
    const value = extract(photo.data);
...
// `unlisted: true` keeps a photo's page (and so its URL) but takes it out of
// every collection: the gallery, the archives, the subjects pages, the
// homepage slideshow, Selected, and the sitemap. It exists because pages are
// never removed or renamed once published (Dermot's ruling, 11 September
// 2026: an outside link may point at them), so this is the only way a photo
// stops showing. Only the literal `true` counts; the validator enforces the
// shape so a quoted "true" or a 1 cannot silently list or unlist a photo.
function isListed(data) {
  return data.unlisted !== true;
}

function listedPhotos(photos) {
  return photos.filter((photo) => isListed(photo.data));
}
